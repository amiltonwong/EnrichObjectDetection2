...
if nargin > 4 && nargin < 7
    ...
    vnum = 8;
    azimuth_invervals = [0 (360/(vnum*2)):(360/vnum):360-(360/(vnum*2))];
end
    ...
    for ground_truth_idx = 1:n_ground_truth
        ...
        if intersection_width > 0 && intersection_height > 0                
            ...
            if use_viewpoint
                ...
                prediction_view_index = find_interval(azimuth_intervals, azimuth_modulo);
                ground_truth_view_index = find_interval(azimuth_intervals, ground_truth_azimuth(ground_truth_idx)); 

                is_correct_view = prediction_view_index == ground_truth_view_index;
    
                % For debugginng, delete below for release
                view_difference = min(...
            ...
            if iou >= iou_max 
                % skip if evaluating viewpoint and the viewpoint is not correct
                if use_viewpoint && ~is_correct_view
                    continue;
                end
                iou_max = iou;
...
% Find bin index of given azimuth, 
function ind = find_interval(azimuth_intervals, query_azimuth)

for i = 1:numel(azimuth_intervals)
    if query_azimuth < azimuth_intervals(i)
        break;
    end
end
...
if query_azimuth > azimuth_intervals(end)
    ind = 1;
end

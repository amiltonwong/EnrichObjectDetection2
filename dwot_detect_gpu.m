...
%   HM = cudaConvFFTData(fhog,templates, param.cuda_conv_n_threads);
  HM = cudaConvolutionFFT(single(hog{level}), maxTemplateHeight, maxTemplateWidth, templates, param.cuda_conv_n_threads, param.device_id);
...
%      end
  ...
  for templateIdx = 1:nTemplates
    ...
    % detection
    [y1, x1] = dwot_hog_to_img_fft(y_coord - 1, x_coord - 1, sz{templateIdx}, sbin, scale);
    [y2, x2] = dwot_hog_to_img_fft(y_coord + sz{templateIdx}(1) + 1, x_coord + sz{templateIdx}(2) + 1, sz{templateIdx}, sbin, scale);

function [best_proposals, detectors, detector_table]= dwot_binary_search_priority_queue(hog_region_pyramid, im_region, detectors, detector_table, renderer, param, im, visualize)
...
for region_idx = 1:n_proposal_region
    ...
    best_state = struct('x',          [curr_detector.az, curr_detector.el, curr_detector.yaw, curr_detector.fov],...
                  'models_idx',       curr_detector.model_index,... % there can be multiple models and renderings 
                  'template_size',    curr_detector.sz,...
                  'rendering_image',  curr_detector.rendering_image,...
...
%   dfov = dfov / 2;
    ...
    % Compute the item in the queue that got the highest score

    queue = [];
    [azs, els, yaws, fovs] = search_viewpoints(curr_detector.az, curr_detector.el, curr_detector.yaw, curr_detector.fov,...
                                                daz, del, dyaw, dfov);
    
	for item_idx = 1:numel(azs)
        item = queue_item(azs(item_idx), els(item_idx), yaws(item_idx), fovs(item_idx), ...
                            curr_detector.model_index, hog_region_pyramid{region_idx}.det_score );
        queue = add_queue(item, queue);
    end
    

    for i = 1:100
        [ item, queue ] = remove_queue(queue);
        curr_detector = dwot_make_detectors(renderer, azs(empty_detectors_indexes),...
                                          els(empty_detectors_indexes),...
                                          yaws(empty_detectors_indexes),...
                                          fovs(empty_detectors_indexes),...
                                          param);
        [max_score, max_template_subset_index, image_bbox] = dwot_detect_region(hog_region_pyramid{region_idx},...
                                                            {curr_detector.whow}, param, im_region{region_idx});
        [azs, els, yaws, fovs] = search_viewpoints(curr_detector.az, curr_detector.el, curr_detector.yaw, curr_detector.fov,...
                                                daz, del, dyaw, dfov);
    
        for item_idx = 1:numel(azs)
            item = queue_item(azs(item_idx), els(item_idx), yaws(item_idx), fovs(item_idx), curr_detector.model_index, hog_region_pyramid{region_idx}.det_score );
            queue = add_queue(item, queue);
        end
    end
...
function [azs, els, yaws, fovs] = search_viewpoints(az, el, yaw, fov, daz, del, dyaw, dfov)
...
idx = 1;
for curr_az = search_angles(az, daz)
    azs(idx) = curr_az;
    idx = idx + 1;
end

for curr_el = search_angles(el, del)
    els(idx) = curr_el;
    idx = idx + 1;
end

for curr_yaw = search_angles(yaw, dyaw)
    yaws(idx) = curr_yaw;
    idx = idx + 1;
end

for curr_fov = search_angles(fov, dfov)
    fovs(idx) = curr_fov;
    idx = idx + 1;
end
...
function angles = search_angles(angle, dangle)
    angles = angle + [-dangle, dangle];
end
...
function item  = queue_item(az,el,yaw,fov,model_idx, score)
    item = struct('x',          [az, el, yaw, fov],...
                  'model_idx',  model_idx,...
                  'score',      score);
end

function queue = add_queue(item, queue)
    queue(end + 1) = item;
    score =  cell2mat({queue.score});
    [~, idx] = sort(score);
    queue = queue(idx);
end

function [item, queue] = remove_queue(queue)
    item = queue(end);
    queue(end) = [];
end

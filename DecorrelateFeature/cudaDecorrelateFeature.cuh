#ifndef CUDA_DECORRELATE_FEATURE_CUH
#define CUDA_DECORRELATE_FEATURE_CUH
...
__global__ void sumAlongFeatures(
    ...
    if(x < WIDTH && y < HEIGHT){
        const int result_i = IMUL(HEIGHT, x) + y;
        const int N = IMUL(WIDTH, HEIGHT);

        result[result_i] = input[result_i];
        for (int z = 1; z < FEATURE_DIM; z++){
            result[result_i] += input[IMUL(z, N) + result_i];
        }
    }
}

__global__ void scrambleGammaToSigma( float* Sigma, 
								const float* Gamma,
								float lambda,
                                const int* nonEmptyRows, const int* nonEmptyCols,
                                int GammaDim, int FEATURE_DIM, int nNonEmptyCells )
{
    int r = blockDim.x * blockIdx.x + threadIdx.x; // rows
    int c = blockDim.y * blockIdx.y + threadIdx.y; // cols 

    int sigmaDim = FEATURE_DIM * nNonEmptyCells;
    if( r < sigmaDim && c < sigmaDim ){
        int HOG_row_idx = r % FEATURE_DIM;
        int HOG_col_idx = c % FEATURE_DIM;

        int currCellIdx = r / FEATURE_DIM;
        int otherCellIdx = c / FEATURE_DIM;
        ...
        Sigma[r + c * sigmaDim] = Gamma[ ((gammaRowIdx * FEATURE_DIM) + HOG_row_idx) + ( ( gammaColIdx * FEATURE_DIM ) + HOG_col_idx ) * GammaDim ]; // + (r==c)?lambda:0 ;

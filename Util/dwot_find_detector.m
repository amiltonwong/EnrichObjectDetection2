function [detectors, template_idx] = dwot_find_detector(detectors_bulk, detector_table, azimuth, elevation, yaw, fov, model_indexes, model_class, param)
% azs  = param.azs;
% els  = param.els;
% yaws = param.yaws;
% fovs = param.fovs;
...
for idx = 1:n_query
	...
	key = dwot_detector_key(az, elevation(idx), yaw(idx), fov(idx), model_indexes);
	if detector_table.isKey(key)
		template_idx(idx) = detector_table( key );
		detectors{idx} = detectors_bulk{template_idx(idx)};
  end
end
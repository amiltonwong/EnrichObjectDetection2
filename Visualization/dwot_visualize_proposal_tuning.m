function dwot_visualize_proposal_tuning(im,...
...
[~, ~, before_tuning_iou, ~] = dwot_evaluate_prediction(...
            dwot_clip_bounding_box(before_tuning_box, size(im)),...
            ground_truth_bounding_boxes, param.min_overlap);

[~, ~, after_tuning_iou, ~] = dwot_evaluate_prediction(...
            dwot_clip_bounding_box(after_tuning_box, size(im)),...
            ground_truth_bounding_boxes, param.min_overlap);
...
[result_im, clipped_bounding_box] = dwot_draw_overlay_rendering(im, bounding_box, rendering, depth, rendering_image_weight);

% Draw
imagesc(result_im); axis equal; axis tight; axis off;

% Draw bounding box annotation on top
dwot_visualize_bounding_boxes(clipped_bounding_box, score, ...
